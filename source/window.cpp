...
void Window::initialize()
{
	...
	window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, program.windowTitle, NULL, NULL);
	...
	GUI_Element::windowSize = glm::vec2(SCREEN_WIDTH, SCREEN_HEIGHT);
	...
	thisWindow->framebuffer_size_callback = [](auto self, int width, int height)
	{
		if (width > 0 && height > 0)
		{
			...
			GUI_Element::windowSize = glm::vec2(self->SCREEN_WIDTH, self->SCREEN_HEIGHT);
